# Import module 
import sqlite3


# Task 1: Create connection object
con = sqlite3.connect('hotel_booking.db')

# Task 2: Create cursor object
curs = con.cursor()

# Task 3: View first row of booking_summary
f_row = curs.execute('''select * from booking_summary ''').fetchone()
# print(f_row)

# Task 4: View first ten rows of booking_summary 
ten_rows = curs.execute('''select * from booking_summary ''').fetchmany(10)

# Task 5: Create object bra and print first 5 rows to view data
bra = curs.execute('''select * from booking_summary where country='BRA' ''').fetchall()
# print(bra)

# Task 6: Create new table called bra_customers
curs.execute('''CREATE TABLE IF NOT EXISTS bra_customers(
  num INTEGER, hotel TEXT, is_cancelled INTEGER, lead_time INTEGER, arrival_date_year INTEGER, arrival_date_month TEXT, arrival_date_day_of_month INTEGER, adults INTEGER, children INTEGER, country TEXT, adr REAL, special_requests INTEGER
)''')

# Task 7: Insert the object bra into the table bra_customers 
curs.executemany(''' INSERT INTO bra_customers VALUES(?,?,?,?,?,?,?,?,?,?,?,?) ''',bra)

# Task 8: View the first 10 rows of bra_customers
b_ten = curs.execute('''select * from  bra_customers''').fetchmany(10)
# print(b_ten)

# Task 9: Retrieve lead_time rows where the bookings were canceled
lead_time_can = curs.execute(''' select lead_time from bra_customers where is_cancelled = 1 ''').fetchall()


# Task 10: Find average lead time for those who canceled and print results
sum_lead_can = 0
average_lead_can = 0
for num in lead_time_can:
  sum_lead_can = sum_lead_can + num[0]
average_lead_can = sum_lead_can/len(lead_time_can) 
print('canceled average',average_lead_can)
# Task 11: Retrieve lead_time rows where the bookings were not canceled
lead_time_not_can = curs.execute(''' select lead_time from bra_customers where is_cancelled = 0 ''').fetchall()

# Task 12: Find average lead time for those who did not cancel and print results
average_not_can = 0
sum_not_can = 0
for num in lead_time_not_can:
  sum_not_can += num[0]
average_not_can = sum_not_can/len(lead_time_not_can)
print('not canceled Average',average_not_can)
# Task 13: Retrieve special_requests rows where the bookings were canceled

sr_not_can = curs.execute(''' select special_requests from bra_customers where is_cancelled = 0 ''').fetchall()
sr_can = curs.execute(''' select special_requests from bra_customers where is_cancelled = 1 ''').fetchall()


# Task 14: Find total speacial requests for those who canceled and print results
special_request_can = 0
for num in sr_can:
  special_request_can +=num[0]
print('total amount of special request for canceled',special_request_can)
# Task 15: Retrieve special_requests rows where the bookings were not canceled
sr_not_can = curs.execute(''' select special_requests from bra_customers where is_cancelled = 0 ''').fetchall()

# Task 16: Find total speacial requests for those who did not cancel and print results
special_request_not_can = 0
for num in sr_not_can:
  special_request_not_can +=num[0]

print('total amount of special request for not canceled',special_request_not_can)

# Task 17: Commit changes and close the connection
con.commit()
con.close()

